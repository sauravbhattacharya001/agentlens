...
AgentLens can capture telemetry in two modes — you pick how the trace gets
produced:

| | **Manual mode** | **Automated mode** |
|---|---|---|
| How telemetry is produced | you **write instrumentation code** — call the SDK to log sessions, decisions, and spans yourself | the **transcript is auto-generated by the agent run** — no instrumentation code to write |
| What you do | wrap the calls you care about | nothing extra; the telemetry falls out of the run |
| Best when | you want fine-grained control over exactly what's traced | you want full coverage with zero boilerplate |

Both modes produce the same kind of trace — sessions, decisions, costs — so the
dashboard and explanations work the same either way. Automated mode just means
you don't hand-write the instrumentation: the agent emits the transcript for you.
