  ...
  const txn = db.transaction(() => {
    for (const group of groups) {
      ...
      const result = insertGroup.run(
        groupId, rule.rule_id, group.label, timestamp,
        JSON.stringify(group.metadata || {}), hash
      );
      ...
      if (result.changes > 0) {
        ...
        newGroups.push({
          group_id: groupId,
          label: group.label,
          member_count: group.events.length,
...
router.post("/schedules", wrapRoute("create/update correlation schedule", (req, res) => {
  ensureSchedulerTables();
  const db = dbMod.getDb();
  const { rule_id, interval_seconds, lookback_minutes, enabled: enabledParam } = req.body;
  ...
  // Verify rule exists
  const rule = db.prepare("SELECT rule_id FROM correlation_rules WHERE rule_id = ?").get(rule_id);
  if (!rule) return res.status(404).json({ error: "Rule not found" });

  const interval = interval_seconds || 300;
  const lookback = lookback_minutes || 60;
  const enabled = enabledParam !== undefined ? (enabledParam ? 1 : 0) : 1;
  ...
  ).run(rule_id, interval, lookback, enabled, nextRun);
...
router.get("/schedules", wrapRoute("list correlation schedules", (req, res) => {
  ensureSchedulerTables();
  const db = dbMod.getDb();
...
router.delete("/schedules/:ruleId", wrapRoute("delete correlation schedule", (req, res) => {
  ensureSchedulerTables();
  const db = dbMod.getDb();
  const result = db.prepare("DELETE FROM correlation_schedules WHERE rule_id = ?").run(req.params.ruleId);
...
router.post("/scheduler/start", wrapRoute("start correlation scheduler", (req, res) => {
  startScheduler();
  res.json({ status: "running" });
}));

/** POST /scheduler/stop — Stop the scheduler loop */
router.post("/scheduler/stop", wrapRoute("stop correlation scheduler", (req, res) => {
  stopScheduler();
  res.json({ status: "stopped" });
}));

/** GET /scheduler/status — Get scheduler status */
router.get("/scheduler/status", wrapRoute("get scheduler status", (req, res) => {
  res.json({
