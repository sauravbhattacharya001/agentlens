    ...
    def _identify_root_causes(self, events: list[dict], errors: list[dict]) -> list[RootCause]:
        ...
        if error_tools:
            ...
            causes.append(RootCause(
                cause_id=f"RC-TOOL-{hashlib.sha256(top_tool.encode()).hexdigest()[:6]}",
        ...
        if error_models:
            ...
            causes.append(RootCause(
                cause_id=f"RC-MODEL-{hashlib.sha256(top_model.encode()).hexdigest()[:6]}",
        ...
        if error_messages:
            ...
            if msg_count >= 2:
                causes.append(RootCause(
                    cause_id=f"RC-REPEAT-{hashlib.sha256(top_msg.encode()).hexdigest()[:6]}",
