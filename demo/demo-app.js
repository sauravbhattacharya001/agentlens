/* ── AgentLens Demo Dashboard — Uses embedded mock data ───────────── */
...
function loadSessions() {
  ...
  if (sessions.length === 0) {
    listEl.innerHTML = '<div class="loading">No sessions match the filter.</div>';
    return;
  }
...
function loadSessionDetail(sessionId) {
  ...
  currentSession = MOCK_SESSIONS.find(s => s.session_id === sessionId);
...
function renderTokenSummary(events) {
  ...
  el.innerHTML = `
    ...
    <table class="token-table">
      <thead>
        <tr><th>Model</th><th>Calls</th><th>Input Tokens</th><th>Output Tokens</th><th>Total</th></tr>
      </thead>
      <tbody>
        ${rows}
...
function loadExplanation(sessionId) {
  const el = document.getElementById("explainContent");
  const explanation = MOCK_EXPLANATIONS[sessionId];
  if (explanation) {
    el.innerHTML = renderMarkdown(explanation);
  } else {
    el.innerHTML = '<div class="loading">No explanation available for this session.</div>';
  }
}
